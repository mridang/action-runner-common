...
- **`ScribeMD/docker-cache@fb28c93`** (0.5.0): Restores cached Docker images on the pre-step and saves them back on the post-step via `actions/cache`. The cache key defaults to `docker-cache-${{ runner.os }}-${{ github.workflow }}` — deliberately excludes `github.job` so all jobs in a workflow share one cache. Different workflows get isolated caches because ScribeMD does not support `restore-keys` and a single global key would either thrash or snowball.
